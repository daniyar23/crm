// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        v6.33.4
// source: proto/company.proto

package grpc

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type GetCompanyByIDRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            uint64                 `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetCompanyByIDRequest) Reset() {
	*x = GetCompanyByIDRequest{}
	mi := &file_proto_company_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetCompanyByIDRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetCompanyByIDRequest) ProtoMessage() {}

func (x *GetCompanyByIDRequest) ProtoReflect() protoreflect.Message {
	mi := &file_proto_company_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetCompanyByIDRequest.ProtoReflect.Descriptor instead.
func (*GetCompanyByIDRequest) Descriptor() ([]byte, []int) {
	return file_proto_company_proto_rawDescGZIP(), []int{0}
}

func (x *GetCompanyByIDRequest) GetId() uint64 {
	if x != nil {
		return x.Id
	}
	return 0
}

type ListCompaniesByUserRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	UserId        uint64                 `protobuf:"varint,1,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListCompaniesByUserRequest) Reset() {
	*x = ListCompaniesByUserRequest{}
	mi := &file_proto_company_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListCompaniesByUserRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListCompaniesByUserRequest) ProtoMessage() {}

func (x *ListCompaniesByUserRequest) ProtoReflect() protoreflect.Message {
	mi := &file_proto_company_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListCompaniesByUserRequest.ProtoReflect.Descriptor instead.
func (*ListCompaniesByUserRequest) Descriptor() ([]byte, []int) {
	return file_proto_company_proto_rawDescGZIP(), []int{1}
}

func (x *ListCompaniesByUserRequest) GetUserId() uint64 {
	if x != nil {
		return x.UserId
	}
	return 0
}

type CompanyResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            uint64                 `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
	Name          string                 `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	UserId        uint64                 `protobuf:"varint,3,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CompanyResponse) Reset() {
	*x = CompanyResponse{}
	mi := &file_proto_company_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CompanyResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CompanyResponse) ProtoMessage() {}

func (x *CompanyResponse) ProtoReflect() protoreflect.Message {
	mi := &file_proto_company_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CompanyResponse.ProtoReflect.Descriptor instead.
func (*CompanyResponse) Descriptor() ([]byte, []int) {
	return file_proto_company_proto_rawDescGZIP(), []int{2}
}

func (x *CompanyResponse) GetId() uint64 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *CompanyResponse) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *CompanyResponse) GetUserId() uint64 {
	if x != nil {
		return x.UserId
	}
	return 0
}

type CompanyListResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Companies     []*CompanyResponse     `protobuf:"bytes,1,rep,name=companies,proto3" json:"companies,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CompanyListResponse) Reset() {
	*x = CompanyListResponse{}
	mi := &file_proto_company_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CompanyListResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CompanyListResponse) ProtoMessage() {}

func (x *CompanyListResponse) ProtoReflect() protoreflect.Message {
	mi := &file_proto_company_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CompanyListResponse.ProtoReflect.Descriptor instead.
func (*CompanyListResponse) Descriptor() ([]byte, []int) {
	return file_proto_company_proto_rawDescGZIP(), []int{3}
}

func (x *CompanyListResponse) GetCompanies() []*CompanyResponse {
	if x != nil {
		return x.Companies
	}
	return nil
}

var File_proto_company_proto protoreflect.FileDescriptor

const file_proto_company_proto_rawDesc = "" +
	"\n" +
	"\x13proto/company.proto\x12\acompany\"'\n" +
	"\x15GetCompanyByIDRequest\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\x04R\x02id\"5\n" +
	"\x1aListCompaniesByUserRequest\x12\x17\n" +
	"\auser_id\x18\x01 \x01(\x04R\x06userId\"N\n" +
	"\x0fCompanyResponse\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\x04R\x02id\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12\x17\n" +
	"\auser_id\x18\x03 \x01(\x04R\x06userId\"M\n" +
	"\x13CompanyListResponse\x126\n" +
	"\tcompanies\x18\x01 \x03(\v2\x18.company.CompanyResponseR\tcompanies2\xb6\x01\n" +
	"\x0eCompanyService\x12J\n" +
	"\x0eGetCompanyByID\x12\x1e.company.GetCompanyByIDRequest\x1a\x18.company.CompanyResponse\x12X\n" +
	"\x13ListCompaniesByUser\x12#.company.ListCompaniesByUserRequest\x1a\x1c.company.CompanyListResponseB>Z<github.com/daniyar23/crm/internal/feature/delivery/grpc;grpcb\x06proto3"

var (
	file_proto_company_proto_rawDescOnce sync.Once
	file_proto_company_proto_rawDescData []byte
)

func file_proto_company_proto_rawDescGZIP() []byte {
	file_proto_company_proto_rawDescOnce.Do(func() {
		file_proto_company_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_proto_company_proto_rawDesc), len(file_proto_company_proto_rawDesc)))
	})
	return file_proto_company_proto_rawDescData
}

var file_proto_company_proto_msgTypes = make([]protoimpl.MessageInfo, 4)
var file_proto_company_proto_goTypes = []any{
	(*GetCompanyByIDRequest)(nil),      // 0: company.GetCompanyByIDRequest
	(*ListCompaniesByUserRequest)(nil), // 1: company.ListCompaniesByUserRequest
	(*CompanyResponse)(nil),            // 2: company.CompanyResponse
	(*CompanyListResponse)(nil),        // 3: company.CompanyListResponse
}
var file_proto_company_proto_depIdxs = []int32{
	2, // 0: company.CompanyListResponse.companies:type_name -> company.CompanyResponse
	0, // 1: company.CompanyService.GetCompanyByID:input_type -> company.GetCompanyByIDRequest
	1, // 2: company.CompanyService.ListCompaniesByUser:input_type -> company.ListCompaniesByUserRequest
	2, // 3: company.CompanyService.GetCompanyByID:output_type -> company.CompanyResponse
	3, // 4: company.CompanyService.ListCompaniesByUser:output_type -> company.CompanyListResponse
	3, // [3:5] is the sub-list for method output_type
	1, // [1:3] is the sub-list for method input_type
	1, // [1:1] is the sub-list for extension type_name
	1, // [1:1] is the sub-list for extension extendee
	0, // [0:1] is the sub-list for field type_name
}

func init() { file_proto_company_proto_init() }
func file_proto_company_proto_init() {
	if File_proto_company_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_proto_company_proto_rawDesc), len(file_proto_company_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   4,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_proto_company_proto_goTypes,
		DependencyIndexes: file_proto_company_proto_depIdxs,
		MessageInfos:      file_proto_company_proto_msgTypes,
	}.Build()
	File_proto_company_proto = out.File
	file_proto_company_proto_goTypes = nil
	file_proto_company_proto_depIdxs = nil
}
